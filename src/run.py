...
def predict(img_input_path, img_output_path, img_input_nodata):
	...
	input_data, input_windows = image_utils.get_chips_padding(img_input_path, size=CHIP_SIZE, start_perc_positions=[(0,0)], rotate=False, flip=False)
...
def evaluate(img_path, nodata_value):
	...
	train_data, test_data, val_data, train_labels, test_labels, val_labels = image_utils.get_input_data(img_path, nodata_value, CHIP_SIZE, SEED)
